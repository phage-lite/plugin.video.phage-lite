...
from modules.providers import RD_CLOUD, FOLDERS, REAL_DEBRID
...
def locations():
    return {
        "navigator_db": "navigator.db",
        "watched_db": "watched.db",
        "favorites_db": "favourites.db",
        "settings_db": "settings.db",
        "trakt_db": "traktcache.db",
        "maincache_db": "maincache.db",
        "metacache_db": "metacache.db",
        "debridcache_db": "debridcache.db",
        "lists_db": "lists.db",
        "tmdb_lists_db": "tmdb_lists.db",
        "discover_db": "discover.db",
        "external_db": "external.db",
        "episode_groups_db": "episode_groups.db",
        "personal_lists_db": "personal_lists.db",
...
def make_databases():
    ...
    if not kodi_utils.path_exists(databases_path):
        kodi_utils.make_directory(databases_path)
...
def clean_databases():
    ...
    clean_cache_list = (
        ("EXTERNAL CACHE", external_cache, database_locations("external_db")),
        ("MAIN CACHE", main_cache, database_locations("maincache_db")),
        ("LISTS CACHE", lists_cache, database_locations("lists_db")),
        ("TMDB LISTS CACHE", lists_cache, database_locations("tmdb_lists_db")),
        ("META CACHE", meta_cache, database_locations("metacache_db")),
    ...
    for item in clean_cache_list:
        ...
        append(
            "[B]%s: [COLOR green]SUCCESS[/COLOR][/B][CR]    [B]Saved Size: %sMB[/B][CR]    Start Size/End Size: %sMB/%sMB"
            % (
                name,
...
def clear_cache(cache_type, silent=False):
    ...
    if cache_type == "meta":
        ...
        results = []
        for item in (external_cache, debrid_cache):
            results.append(item.clear_cache())
        success = False not in results
...
def clear_all_cache():
    ...
    caches = (
        ("meta", "Meta Cache"),
        ("internal_scrapers", "Internal Scrapers Cache"),
        ("external_scrapers", "External Scrapers Cache"),
        ("trakt", "Trakt Cache"),
        ("imdb", "IMDb Cache"),
        ("list", "List Data Cache"),
        ("ai_functions", "AI Data Cache"),
        ("tmdb_list", "TMDb Personal List Cache"),
        ("main", "Main Cache"),
        (RD_CLOUD, "%s Cloud" % REAL_DEBRID),
    )
...
def refresh_cached_data(meta):
    ...
    try:
        meta_cache.delete(media_type, "tmdb_id", tmdb_id, meta)
    except:
...
def columns_in_table(database, table, check_existence=""):
    ...
    all_columns = [
        i[1] for i in dbcon.execute("PRAGMA table_info(%s);" % table).fetchall()
    ]
    if check_existence:
        return check_existence in all_columns
    return all_columns
...
def insert_new_column_in_table(database, table, new_column, new_column_properties):
    try:
        ...
        dbcon.execute(
            ...
            % (table, new_column, new_column_properties)
        )
        return True
    except:
        return False
...
def check_and_insert_new_columns(database, table, new_column, new_column_properties):
    ...
    try:
        if not columns_in_table(database, table, new_column):
            success = insert_new_column_in_table(
                database, table, new_column, new_column_properties
            )
            if not success:
                kodi_utils.notification(
                    "Error with [B]%s[/B] Database. Missing Column [B]%s[/B]"
                    % (database.upper(), new_column.upper())
                )
    except:
        kodi_utils.notification(f"Error Checking Database Table/s: {database}")
...
class BaseCache(object):
    ...
    def get(self, string):
        ...
        try:
            ...
            cache_data = dbcon.execute(
                "SELECT expires, data FROM %s WHERE id = ?" % self.table, (string,)
            ).fetchone()
            if cache_data:

...
class source:
    ...
    def results(self, info):
        try:
            ...
            self.media_type, title, self.tmdb_id = (
                info.get("media_type"),
                info.get("title"),
                info.get("tmdb_id"),
            )
            self.year, self.season, self.episode = (
                int(info.get("year")),
                info.get("season"),
                info.get("episode"),
            )
            ...
            def _process():
                for item in self.scrape_results:
                    try:
                        ...
                        file_dl, size = (
                            item["url_link"],
                            round(float(item["bytes"]) / 1073741824, 2),
                        )
        ...
        try:
            ...
            for item in my_cloud_files:
                ...
                if not folder_name:
                    ...
                    if self.media_type == "movie" and not any(
                        x in normalized for x in year_query_list
                    ):
                        continue
                    results_append(item["id"])
    ...
    def _scrape_folders(self, folder_info):
        try:
            ...
            contents = [
                i
                for i in folder_files["files"]
                if i["selected"] == 1
                and i["path"].lower().endswith(tuple(self.extensions))
            ]
            ...
            for item in contents:
                ...
                if self.media_type == "episode" and not source_utils.seas_ep_filter(
                    self.season, self.episode, normalized
                ):
                    continue
                if item["path"].replace("/", "").lower() not in [
                    d["path"].replace("/", "").lower() for d in self.scrape_results
                ]:
            ...
            for item in my_downloads:
                ...
                if self.media_type == "movie":
                    if not any(x in normalized for x in year_query_list):
                        continue
                elif not source_utils.seas_ep_filter(
                    self.season, self.episode, normalized
                ):
                    continue

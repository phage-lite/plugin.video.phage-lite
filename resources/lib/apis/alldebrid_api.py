...
from modules.kodi_utils import progress_dialog, notification, hide_busy_dialog, show_busy_dialog, sleep, ok_dialog
...
class AllDebridAPI:
	...
	def check_cache(self, hashes):
		return self._post('magnet/instant', {'magnets[]': hashes})

	def check_single_magnet(self, hash_string):
		return self.check_cache(hash_string)['magnets'][0]['instant']

	def user_cloud(self):
		return cache_object(self._get, 'ad_user_cloud', 'magnet/status', False, 0.03)

	def history(self):
		return cache_object(self._get, 'ad_user_history', 'user/history', False, 0.03)

	def user_links(self):
		return cache_object(self._get, 'ad_user_links', 'user/links', False, 0.03)

	def unrestrict_link(self, link):
		response = self._get('link/unlock', '&link=%s' % quote(link))
		try: return response['link']
		except: return None

	def create_transfer(self, magnet):
		result = self._get('magnet/upload', '&magnet=%s' % magnet)
		if 'error' in result: return None
		return result['magnets'][0].get('id', None)

	def list_transfer(self, transfer_id):
		return self._get('magnet/status', '&id=%s' % transfer_id)['magnets']

	def delete_transfer(self, transfer_id):
		result = self._get('magnet/delete', '&id=%s' % transfer_id)
			...
			if valid_results:
				if season:
					...
					if correct_files:
						...
						try: media_id = [i['l'] for i in correct_files if not any(x in re.sub(episode_title, '', seas_ep_filter(season, episode, i['n'], split=True)) for x in _extras)][0]
	...
	def _get(self, url, url_append=''):
		...
		try:
			full_url = self.base_url + url + '?agent=%s&apikey=%s' % (self.user_agent, self.token) + url_append
			if 'magnet/status' in full_url: full_url = full_url.replace('v4', 'v4.1')
			result = requests.get(full_url, timeout=20).json()
	...
	def _post(self, url, data=None):
		...
		try:
			full_url = self.base_url + url + '?agent=%s&apikey=%s' % (self.user_agent, self.token)
			result = requests.post(full_url, data=data, timeout=20).json()
			if result.get('status') == 'success' and 'data' in result: result = result['data']
			return result
		except: return None

...
class EasyNewsAPI:
	...
	def _get_auth(self):
		...
		auth = 'Basic ' + base64.b64encode(user_info.encode('utf-8')).decode('utf-8')
		return auth
	...
	def process_image_files(self, files):
			...
			for item in files:
				try:
					...
					result = {'name': '%s_%s_%02d' % (self.query, post_title, count),
							  'fullsize': size, 'fullres': item['fullres'],
							  'url_dl': url_dl, 'down_url': file_dl,
							  'version': 'version2', 'thumbnail': thumbnail, 'group': group}
					count += 1
					yield result
		...
		results = list(chunks(list(_process()), 50))
		return {'total_results': len(results), 'total_pages': len(results), 'results': results}
		...
		def _process():
			for item in files:
				try:
					...
					short_vid = bool(re.match(r'^\d+s', duration) or re.match(r'^[0-5]m', duration))
					url_add = quote('/%s/%s/%s%s/%s%s' % (dl_farm, dl_port, post_hash, ext, post_title, ext))
					stream_url = streaming_url + url_add
					file_dl = down_url + url_add + '|Authorization=%s' % self.auth_quoted
					thumbnail = 'https://th.easynews.com/thumbnails-%s/pr-%s.jpg' % (post_hash[0:3], post_hash)
					yield {'name': post_title, 'size': size, 'rawSize': item['rawSize'],
						   'width': int(item['width']), 'runtime': int(item['runtime']/60.0),
						   'url_dl': stream_url, 'down_url': file_dl, 'version': 'version2',
						   'short_vid': short_vid, 'language': language, 'thumbnail': thumbnail}
				except Exception as e:
					from modules.kodi_utils import logger
					logger('easynews API Exception', str(e))
		down_url = files.get('downURL')
	...
	def _translate_search(self, search_type='VIDEO'):
		...
		'fty[]': 'IMAGE', 'pby': 10000}
		params = {'VIDEO': SEARCH_PARAMS, 'IMAGE': IMAGE_SEARCH_PARAMS}[search_type]
		params['safeO'] = 0
		params['gps'] = self.query
		return self.base_url + self.search_link, params
	...
	def resolve_easynews(self, url_dl, use_non_seekable=False):
		...
		return response.url + ('|seekable=0' if use_non_seekable else '')

...
def date_difference(
    current_date, compare_date, difference_tolerance, allow_postive_difference=False
):
    try:
...
def remove_accents(obj):
    try:
        ...
        obj = "".join(
            c
            for c in unicodedata.normalize("NFD", obj)
...
def regex_from_to(text, from_string, to_string, excluding=True):
    if excluding:
        ...
        r = re.search(
            r"(?i)(" + from_string + r"[\S\s]+?" + to_string + ")", text
        ).group(1)
    return r
...
def sort_for_article(_list, _key, ignore_articles):
    try:
        if not ignore_articles:
            _list.sort(key=lambda k: k.get(_key))
        else:
            _list.sort(
                key=lambda k: re.sub(r"(^the |^a |^an )", "", k.get(_key).lower())
            )
    except Exception:
        pass
    return _list
...
def sort_list(sort_key, sort_direction, list_data, ignore_articles):
    try:
        ...
        if sort_key == "title":
            return sorted(
                list_data,
                key=lambda x: title_key(x[x["type"]].get("title"), ignore_articles),
                reverse=reverse,
            )
...
def image_from_db(image_url, delete=True):
    ...
    try:
        ...
        try:
            image_id, image_location = dbcur.execute(
                "SELECT id, cachedurl FROM texture WHERE url = ?", (image_url,)
            ).fetchone()
        except Exception:
            return True
...
def make_image(list_type, image_type, list_name, images, current_image):
    ...
    def _process(count, item):
        saved_path = translate_path(
            os.path.join(worker_image_folder, "%s_%02d.jpg" % (list_name, count + 1))
        )
    ...
    try:
        ...
        worker_image_folder = os.path.join(
            profile_path, "images", "%s_worker" % list_type
        )
        final_image_folder = os.path.join(
            profile_path, "images", "%s_%s" % (list_type, image_type)
        )
        saved_final_image = os.path.join(
            final_image_folder, "%s_%s.jpg" % (md5_image_name, get_current_timestamp())
        )
        for location in (worker_image_folder, final_image_folder):
            make_directory(location)
        if image_type == "poster":
            new_dimensions, size_dimensions, placements = (
                (1000, 1500),
                (500, 750),
                ((0, 0), (500, 0), (0, 750), (500, 750)),
            )
        else:
            new_dimensions, size_dimensions, placements = (
                (1280, 720),

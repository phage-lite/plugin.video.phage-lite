...
def get_key_id(params):
    ...
    if search_type == "media_title":
        if media_type == "movie":
            url_params, string = (
                {"mode": "build_movie_list", "action": "tmdb_movies_search"},
                "movie_queries",
            )
            ...
            url_params, string = (
                {
                    ...
                    "action": "trakt_anime_search",
                    "is_anime_list": "true",
                },
                "anime_queries",
            )
        elif media_type == "tvshow_anime":
            url_params, string = (
                {"mode": "build_tvshow_list", "action": "tmdb_tv_search"},
                "tvshow_anime_queries",
            )
        else:
            url_params, string = (
                {"mode": "build_tvshow_list", "action": "trakt_tv_search"},
                "tvshow_queries",
            )
        ...
        url_params, string = (
            {"mode": "navigator.keyword_results", "media_type": media_type},
            "keyword_tmdb_%s_queries" % media_type,
        )
    elif search_type == "easynews_video":
        url_params, string = (
            {"mode": "easynews.search_easynews"},
            "easynews_video_queries",
        )
    elif search_type == "easynews_image":
        url_params, string = (
            {"mode": "easynews.search_easynews_image"},
    ...
    if string:
        add_to_search(key_id, string)
    if search_type == "people":
        return person_search(key_id)
    if search_type == "easynews_image":
        return search_easynews_image(key_id)
    url_params.update(
        {"query": key_id, "key_id": key_id, "name": "Search Results for %s" % key_id}
    )
    return execute_builtin(
        "ActivateWindow(Videos,%s,return)"
        if external()
        else "Container.Update(%s)" % build_url(url_params)
    )
...
def clear_search():
    clear_history_list = [
        ("Clear Movie Search History", "movie_queries"),

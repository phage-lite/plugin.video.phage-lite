    ...
    if action == "image":
        ...
        try:
            ...
            return kodi_utils.notification(
                "No URL found for Download. Pick another Source."
            )
        ...
        default_foldername = kodi_utils.kodi_dialog().input(
            "Title", defaultt=default_name
        )
        multi_downloads = []
        ...
        for item in chosen_list:
            ...
            multi_downloads_append(
                (
                    Thread(target=Downloader(item).run),
                    clean_file_name(item["pack_files"]["filename"]),
                )
            )
...
def download_threads_manager(multi_downloads, image):
    ...
    for item in multi_downloads:
        ...
        remaining_downloads = [
            x[1] for x in multi_downloads if not x in started_downloads
        ]
        kodi_utils.set_property(
            "bacterio.active_queued_downloads", json.dumps(remaining_downloads)
        )
    ...
    list_items = [
        {
            "line1": "%.2f GB | %s"
            % (
                float(item["pack_files"]["size"]) / 1073741824,
                clean_file_name(item["pack_files"]["filename"]).upper(),
            ),
            "icon": icon,
        }
        for item in pack_choices
    ]
    heading = "Choose Files to Download - %s" % clean_file_name(
...
def get_title(meta):
    title = (
        meta.get("custom_title", None) or meta.get("english_title") or meta.get("title")
    )
    return title
...
class Downloader:
    ...
    def run(self):
        ...
        if self.url in (None, "None", ""):
            return self.return_notification(
                _notification="No URL found for Download. Pick another Source"
            )
        self.get_filename()
        self.get_extension()
        ...
        if not self.confirm_download():
            return self.return_notification(_notification="Cancelled")
    ...
    def add_active_download(self):
        ...
        kodi_utils.set_property(
            "bacterio.active_downloads", json.dumps(active_downloads)
        )
    ...
    def remove_active_download(self):
        ...
        active_downloads = self.get_active_downloads()
        try:
            active_downloads.remove(self.final_name)
        except:
            pass
        if active_downloads:
            kodi_utils.set_property(
    ...
    def check_status(self):
        status = kodi_utils.get_property(
            "bacterio.download_status.%s" % self.final_name
        )
        if status in ("unpaused", "cancelled"):
            kodi_utils.clear_property("bacterio.download_status.%s" % self.final_name)
        return status
    ...
    def get_url_and_headers(self):
        ...
        if url in (None, "None", ""):
            if self.action == "meta.single":
                ...
                if self.provider in (REAL_DEBRID):
    ...
    def get_download_folder(self):
        ...
        for level in ["../../../..", "../../..", "../..", ".."]:
            try:
                kodi_utils.make_directory(
                    os.path.abspath(os.path.join(self.down_folder, level))
                )
            except:
                pass
    ...
    def get_destination_folder(self):
        if self.action == "image":
            ...
            if self.action == "meta.single":
                folder_rootname = kodi_utils.kodi_dialog().input(
                    "Title", defaultt=default_name
                )
            else:
    ...
    def get_filename(self):
        if self.final_name:
            ...
            if clean_title(self.title).lower() in file_name.lower():
                ...
                try:
                    final_name = self.name.translate(None, r'\/:*?"<>|').strip(".")
                except:
                    final_name = os.path.splitext(urlparse(name_url).path)[0].split(
                        "/"
                    )[-1]
    ...
    def get_extension(self):
        if self.action == "archive":
            ...
            if not ext in kodi_utils.image_extensions():
                ext = "jpg"
        else:
            ext = os.path.splitext(urlparse(self.url).path)[1][1:]
            if not ext in kodi_utils.video_extensions():
                ext = "mp4"
        ext = ".%s" % ext
        self.extension = ext
    ...
    def finish_download(self, status):
        if self.action == "image":
            if self.media_type == "image_url":
                return kodi_utils.notification(
                    status.upper(), 2500, self.final_destination
                )
            else:
                return
        if not kodi_utils.get_visibility("Window.IsActive(fullscreenvideo)"):
            kodi_utils.notification(
                "[B]%s[/B] %s"
                % (status.upper(), self.final_name.replace(".", " ").replace("_", " ")),
                2500,
                self.image,
            )
    ...
    def confirm_download(self):
        return (
            ...
            else kodi_utils.confirm_dialog(
                heading=self.final_name,
                text="Complete file is [B]%dMB[/B][CR]Continue with download?"
                % self.mb,
            )
        )
    ...
    def return_notification(self, _notification=None, _ok_dialog=None):
        kodi_utils.hide_busy_dialog()
        if _notification:
            kodi_utils.notification(_notification, 2500)
        elif _ok_dialog:
            kodi_utils.ok_dialog(text=_ok_dialog)
        else:
            return
...
def manager():
    ...
    kwargs = {}
    return open_window(
        ("windows.downloads_manager", "DownloadsManager"),
        "downloads_manager.xml",
        **kwargs,
    )

...
class Movies:
        ...
        try:
            ...
            if page_no == 1 and not self.is_external:
                kodi_utils.set_property(
                    "bacterio.exit_params", kodi_utils.folder_path()
                )
            ...
            if (
                self.total_pages
                and self.total_pages > 2
                and settings.jump_to_enabled()
            ...
            kodi_utils.logger("fetch_list", traceback.format_exc())
            pass
    ...
    def build_movie_content(self, _position, _id):
        try:
            ...
            browse_related_params = self.build_url(
                {
                    "mode": "build_movie_list",
                    "action": "trakt_movies_related",
                    "key_id": imdb_id,
                    "is_external": self.is_external,
            ...
            cm_append(
                [
                    "playback_options",
                    ("[B]Play Options[/B]", "RunPlugin(%s)" % playback_options_params),
                ]
            )
            if (
            ...
            cm_append(
                [
                    ...
                    (
                        "[B]Browse Related[/B]",
                        self.window_command % browse_related_params,
                    ),
                ]
            )
            cm_append(
    ...
    def worker(self):
        ...
        self.open_movieset = open_action in ("Open Movie Set", "Both") and not self.movieset_list_active
        self.open_extras = open_action in ("Open Extras", "Both")
